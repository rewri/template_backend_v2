...
import { FindEmployeeResponseDTO } from './find-employee.dto';
...
export class FindEmployeeService {
  ...
  async findEmployee(id: number): Promise<FindEmployeeResponseDTO> {
    try {
      ...
      return FindEmployeeResponseDTO.fromEntity(employee);
    } catch (error) {
